...
func init() {
	...
	accountGRPCHost := os.Getenv("PN_RETM_ACCOUNT_GRPC_HOST")
	accountGRPCPort := os.Getenv("PN_RETM_ACCOUNT_GRPC_PORT")
	accountGRPCAPIKey := os.Getenv("PN_RETM_ACCOUNT_GRPC_API_KEY")
	friendsGRPCHost := os.Getenv("PN_RETM_FRIENDS_GRPC_HOST")
	friendsGRPCPort := os.Getenv("PN_RETM_FRIENDS_GRPC_PORT")
	friendsGRPCAPIKey := os.Getenv("PN_RETM_FRIENDS_GRPC_API_KEY")
	healthCheckPort := os.Getenv("PN_RETM_HEALTH_CHECK_PORT")
	...
	if port, err := strconv.Atoi(authenticationServerPort); err != nil {
		globals.Logger.Errorf("PN_RETM_AUTHENTICATION_SERVER_PORT is not a valid port. Expected 0-65535, got %s", authenticationServerPort)
		os.Exit(0)
	...
	if strings.TrimSpace(secureServerHost) == "" {
		globals.Logger.Error("PN_RETM_SECURE_SERVER_HOST environment variable not set")
		os.Exit(0)
	}

	if strings.TrimSpace(secureServerPort) == "" {
		globals.Logger.Error("PN_RETM_SECURE_SERVER_PORT environment variable not set")
		os.Exit(0)
	}

	if port, err := strconv.Atoi(secureServerPort); err != nil {
		globals.Logger.Errorf("PN_RETM_SECURE_SERVER_PORT is not a valid port. Expected 0-65535, got %s", secureServerPort)
		os.Exit(0)
	} else if port < 0 || port > 65535 {
		globals.Logger.Errorf("PN_RETM_SECURE_SERVER_PORT is not a valid port. Expected 0-65535, got %s", secureServerPort)
		os.Exit(0)
	...
	if strings.TrimSpace(accountGRPCHost) == "" {
		globals.Logger.Error("PN_RETM_ACCOUNT_GRPC_HOST environment variable not set")
		os.Exit(0)
	}

	if strings.TrimSpace(accountGRPCPort) == "" {
		globals.Logger.Error("PN_RETM_ACCOUNT_GRPC_PORT environment variable not set")
		os.Exit(0)
	}
	...
	if strings.TrimSpace(friendsGRPCPort) == "" {
		globals.Logger.Error("PN_RETM_FRIENDS_GRPC_PORT environment variable not set")
		os.Exit(0)
	}
	if port, err := strconv.Atoi(friendsGRPCPort); err != nil {
		globals.Logger.Errorf("PN_RETM_FRIENDS_GRPC_PORT is not a valid port. Expected 0-65535, got %s", accountGRPCPort)
		os.Exit(0)
	} else if port < 0 || port > 65535 {
		globals.Logger.Errorf("PN_RETM_FRIENDS_GRPC_PORT is not a valid port. Expected 0-65535, got %s", accountGRPCPort)
		os.Exit(0)
	...
	if strings.TrimSpace(friendsGRPCAPIKey) == "" {
		globals.Logger.Warning("Insecure gRPC server detected. PN_RETM_FRIENDS_GRPC_API_KEY environment variable not set")
	}
	...
	if strings.TrimSpace(healthCheckPort) == "" {
		globals.Logger.Warning("Basic UDP health check will not be enabled. PN_RETM_HEALTH_CHECK_PORT environment variable not set")
	} else if port, err := strconv.Atoi(healthCheckPort); err != nil {
		globals.Logger.Errorf("PN_RETM_HEALTH_CHECK_PORT is not a valid port. Expected 0-65535, got %s", healthCheckPort)
		os.Exit(0)
	} else if port < 0 || port > 65535 {
		globals.Logger.Errorf("PN_RETM_HEALTH_CHECK_PORT is not a valid port. Expected 0-65535, got %s", healthCheckPort)
		os.Exit(0)
	...
	globals.Postgres, err = sql.Open("postgres", os.Getenv("PN_RETM_POSTGRES_URI"))
	if err != nil {
